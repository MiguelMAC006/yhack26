...
function onLandmarksUpdate(landmarks) {
    if (isInitialized && isFaceTracking && landmarks) {
        allLandmarks = landmarks;
        
        // Filter landmarks for first and second point sets
        firstSetPosition = [];
        for (var i = 0; i < firstSelectedPoints.length; i++) {
            var idx1 = firstSelectedPoints[i];
            firstSetPosition[i] = (idx1 < landmarks.length) ? landmarks[idx1] : null;
        }
        
        secondSetPosition = [];
        for (var j = 0; j < secondSelectedPoints.length; j++) {
            var idx2 = secondSelectedPoints[j];
            secondSetPosition[j] = (idx2 < landmarks.length) ? landmarks[idx2] : null;
        }
...
function getLandmarkCenter(points) {
    ...
    for (var i = 0; i < points.length; i++) {
        if (points[i] != undefined && points[i] != null) {
    ...
    if (validCount > 0) {
        center.x = center.x / validCount;
        center.y = center.y / validCount;
    }
