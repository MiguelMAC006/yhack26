...
function onLandmarksUpdate(landmarks) {
    if (isInitialized && isFaceTracking && landmarks) {
        // Filter landmarks to only include selectedPoints
        for (var i = 0; i < selectedPoints.length; i++) {
            var idx = selectedPoints[i];
            landmarksPoint[i] = (idx < landmarks.length) ? landmarks[idx] : null;
        }
        pinToFaceLandmarks();
    }
}
