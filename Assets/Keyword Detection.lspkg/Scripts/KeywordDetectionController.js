...
// Store latest result
var lastDetectedLabel = "";
var lastDetectedProb = 0.0;

// Store best result seen during the current attempt/session
var bestDetectedProbByLabel = {};
...
function onRunningFinished() {
    var classIdx = getDetectedClassId(output.data);
    var classProb = output.data[classIdx];
    var detectedLabel = labels[classIdx];

    lastDetectedLabel = detectedLabel;
    print(lastDetectedLabel);
    print(classProb);
    print("1234");
    lastDetectedProb = classProb;
    print("scoreText:" + script.scoreText);
    var lowerLabel = detectedLabel.toLowerCase();
    if (bestDetectedProbByLabel[lowerLabel] === undefined || classProb > bestDetectedProbByLabel[lowerLabel]) {
        bestDetectedProbByLabel[lowerLabel] = classProb;
    }
    if (lastDetectedLabel == "dog") {
        script.scoreText.text = "SCORE:" + (1000 * classProb.toFixed(2));
    } else {
    ...
    if (classProb >= script.threshold) {
        updateDebug(detectedLabel, classProb.toFixed(2));

        if (script.useBehavior) {
            var customTrigger = script.customTriggerPrefix + detectedLabel.toUpperCase();
            global.behaviorSystem.sendCustomTrigger(customTrigger);
        }
    } else {
        updateDebug("", "");
...
script.api.getLastDetectedLabel = function() {
    return lastDetectedLabel;
};

script.api.getLastDetectedProb = function() {
    return lastDetectedProb;
};

script.api.getAudioScoreForWord = function(targetWord) {
    if (!targetWord) {
        return 0.0;
    }

    if (lastDetectedLabel && lastDetectedLabel.toLowerCase() === targetWord.toLowerCase()) {
        return lastDetectedProb;
    }
...
script.api.getBestAudioScoreForWord = function(targetWord) {
    if (!targetWord) {
        return 0.0;
    }

    var lowerTarget = targetWord.toLowerCase();
    if (bestDetectedProbByLabel[lowerTarget] !== undefined) {
        return bestDetectedProbByLabel[lowerTarget];
    }

    return 0.0;
};

script.api.resetAudioScores = function() {
    bestDetectedProbByLabel = {};
    lastDetectedLabel = "";
    lastDetectedProb = 0.0;
};
